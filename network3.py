...
class Network:
    ...
    def feedforward(self, a):
        ...
        for i, (b, w) in enumerate(zip(self.biases, self.weights), 2):
            if i == self.num_layers:
                a = self.output_af.f(w @ a + b)
            else:
                a = self.hidden_af.f(w @ a + b)
    ...
    def backprop(self, x, y):
        ...
        for i, (b, w) in enumerate(zip(self.biases, self.weights), 2):
            ...
            zs.append(z)
            if i == self.num_layers:
                activation = self.output_af.f(z)
            else:
                activation = self.hidden_af.f(z)
            activations.append(activation)

        # cost_derivative(コスト関数の勾配、すなわち出力層の誤差)
        delta = (self.cost).nabla(zs[-1], activations[-1], y) * self.output_af.prime(zs[-1])
        nabla_b[-1] = delta
        nabla_w[-1] = delta @ activations[-2].T

        for l in range(2, self.num_layers):
            z = zs[-l]
            prime = self.hidden_af.prime(z)
            delta = self.weights[-(l-1)].T @ delta * prime
            nabla_b[-l] = delta
